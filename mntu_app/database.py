...
def init_db():
    ...
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS announcement_delivery_mutes (
    ...
    """)
    _ensure_table_column(c, "schedule_data", "source_file_name", "TEXT DEFAULT ''")
    c.execute("""
...
def save_schedule_to_db(
    ...
    c.execute(
        ...
        INSERT INTO schedule_data (
            schedule_json, group_options, week_options, week_dates, saved_at, source_file_name
        ) VALUES (?, ?, ?, ?, ?, ?)
        """,
        (
            schedule_json,
...
def load_schedule_from_db():
    ...
    c.execute(
        """
        SELECT schedule_json, group_options, week_options, week_dates, saved_at, source_file_name
        FROM schedule_data
        ORDER BY id DESC
        LIMIT 1
        """
...
def save_announcement(
    ...
    announcement_id = int(c.lastrowid or 0)
    conn.commit()
    conn.close()
    return announcement_id


def _get_skipped_announcement_delivery_ids() -> set[int]:
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("SELECT announcement_id FROM announcement_delivery_mutes")
    rows = c.fetchall()
    conn.close()
    return {int(r[0]) for r in rows if r and r[0] is not None}


def skip_announcement_delivery(announcement_id: int):
    aid = int(announcement_id or 0)
    if aid <= 0:
        return
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute(
        "INSERT OR IGNORE INTO announcement_delivery_mutes (announcement_id) VALUES (?)",
        (aid,),
    )
    conn.commit()
    conn.close()
...
def get_unread_announcements(target_group: str = "", include_meta: bool = False):
    ...
    rows = c.fetchall()
    conn.close()
    skipped_ids = _get_skipped_announcement_delivery_ids()
    out = []
    group = (target_group or "").strip()
    for aid, raw_message, created_at in rows:
        if int(aid) in skipped_ids:
            continue
...
NOTIF_KIND_PRE = "pre"
NOTIF_KIND_START = "start"
NOTIF_SUBJECT_SEP = "::"


def notification_storage_subject(subject: str, kind: str = NOTIF_KIND_PRE) -> str:
    return f"{subject}{NOTIF_SUBJECT_SEP}{kind}"


def parse_notification_storage_key(date_str: str, time_str: str, stored_subject: str) -> tuple[str, str, str, str]:
    if NOTIF_SUBJECT_SEP in (stored_subject or ""):
        subject, kind = stored_subject.rsplit(NOTIF_SUBJECT_SEP, 1)
        return (date_str, time_str, subject, kind)
    return (date_str, time_str, stored_subject, NOTIF_KIND_PRE)
...
def get_shown_notifications_for_today():
    ...
    rows = c.fetchall()
    conn.close()
    return {parse_notification_storage_key(r[0], r[1], r[2]) for r in rows}


def mark_notification_shown(date_str: str, time_str: str, subject: str, kind: str = NOTIF_KIND_PRE):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    today = datetime.now().date().isoformat()
    stored_subject = notification_storage_subject(subject, kind)
    try:
        c.execute(
            "INSERT OR IGNORE INTO shown_notifications (date_str, time_str, subject, notification_date) VALUES (?, ?, ?, ?)",
            (date_str, time_str, stored_subject, today),
        )
        conn.commit()
    except Exception:
        pass
...
def get_user_profile_by_email(email: str) -> dict | None:
    ...
    c.execute(
        """
        SELECT email, last_name, first_name, middle_name, role, created_at, updated_at
    ...
    return {
        ...
        "role": row[4] or "Студент",
        "created_at": row[5] or "",
        "updated_at": row[6] or "",
    }


def list_users(limit: int = 200, search: str = "") -> list[dict]:
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    ...
    if q:
        ...
        c.execute(
            ...
            SELECT email, last_name, first_name, middle_name, role, created_at, updated_at
            FROM users
            WHERE lower(email) LIKE ? OR lower(last_name) LIKE ? OR lower(first_name) LIKE ?

...
import requests
from mntu_app.database import is_admin_email, get_pending_role_requests, review_role_request
...
class TelegramBridge:
    ...
    def _main_keyboard(self) -> dict:
        return {
            "keyboard": [
                [{"text": "👤 Профіль"}, {"text": "📊 Статус"}],
                [{"text": "👥 Групи"}, {"text": "🔄 Змінити групу"}],
                [{"text": "🕒 Змінити час"}, {"text": "📢 Надіслати повідомлення"}],
                [{"text": "🔎 Парсинг груп"}],
                [{"text": "🛡 Заявки (адмін)"}],
    ...
    def _build_profile_message(self, state: dict) -> str:
        last = (state.get("last_name") or "").strip()
        first = (state.get("first_name") or "").strip()
        middle = (state.get("middle_name") or "").strip()
        fio = " ".join([last, first, middle]).strip() or "не вказано"
        email = (state.get("email") or "").strip() or "не вказано"
        role = (state.get("user_role") or "Студент").strip()
        group = (state.get("notify_group") or "").strip() or "не вибрано"
        notifications = "увімкнено" if state.get("notifications_enabled", True) else "вимкнено"
        minutes = state.get("notify_minutes", 10)
        telegram_app = "увімкнено" if state.get("telegram_enabled") else "вимкнено"
        chat_linked = "так" if (state.get("telegram_chat_id") or "").strip() else "ні"
        app_access = "відкрито"
        if state.get("auth_required") and not state.get("tg_auth_verified"):
            app_access = "потрібен вхід у додаток"
        return (
            "👤 Ваш профіль\n\n"
            f"ПІБ: {fio}\n"
            f"Email: {email}\n"
            f"Роль: {role}\n"
            f"Група для сповіщень: {group}\n"
            f"Сповіщення: {notifications}\n"
            f"Час нагадування: {minutes} хв\n"
            f"Telegram у додатку: {telegram_app}\n"
            f"Чат бота прив'язано: {chat_linked}\n"
            f"Доступ до додатку: {app_access}"
        )
    ...
    def _handle_update(self, token: str, update: dict):
        ...
        is_admin = role == "Адміністратор" or is_admin_email(user_email)
        ...
        if cmd in ("/start", "/help"):
            self._send_message(
                ...
                "/profile - ваш профіль (ПІБ, email, роль)\n"
        ...
        if cmd == "/login":
            ...
            return
        if cmd == "/profile":
            self._send_message(token, chat_id, self._build_profile_message(state))
            return
        if cmd == "/status":
            self._send_message(
                token,

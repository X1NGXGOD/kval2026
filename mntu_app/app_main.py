...
def main(page: ft.Page):
    ...
    page.window.min_height = 730
    page.window.max_width = 990
    ...
    schedule_data, group_options, week_options, week_dates, schedule_saved_at, schedule_source_file = load_schedule_from_db()
    if not schedule_has_entries(schedule_data):
        schedule_data, group_options, week_options, week_dates = {}, [], [], {}
        schedule_saved_at, schedule_source_file = "", ""
    has_cached_schedule = schedule_has_entries(schedule_data)
    ...
    state = {
        ...
        "schedule_from_cache": has_cached_schedule,
        "schedule_saved_at": schedule_saved_at or "",
        "schedule_source_file": schedule_source_file or "",
        "schedule_browsing": not has_cached_schedule,
        "viewing_images": None,
    ...
    if current_email:
        if is_admin_email(current_email):
    ...
    def apply_bot_updates(updates: dict):
        ...
        if "announcement_text" in updates:
            ...
            if text:
                aid = save_announcement(text)
                finish_announcement_notification(
                    aid,
                    telegram_text=f"📢 Повідомлення від бота:\n{text}",
                    popup_body=text[:200],
                    snack_text=f"📢 {text}",
                )
                changed = True
        if "announcement_payload" in updates:
            ...
            if title or text:
                ...
                finish_announcement_notification(
                    aid,
                    telegram_text="\n".join(preview_parts),
                    popup_body=local_msg[:200],
                    snack_text=f"📢 {local_msg}",
                )
                changed = True
        if changed:
            save_settings_to_db(
    ...
    def finish_announcement_notification(
        announcement_id: int,
        *,
        telegram_text: str = "",
        popup_title: str = "Нове повідомлення",
        popup_body: str = "",
        snack_text: str = "",
    ):
        if not state.get("notifications_enabled", True):
            if announcement_id:
                skip_announcement_delivery(announcement_id)
            return
        if telegram_text:
            telegram_bridge.notify(telegram_text)
        if not state.get("ui_ready"):
            return
        if popup_body:
            show_system_notification(popup_title, popup_body[:200], timeout=8)
        if snack_text:
            show_snack(page, snack_text)

    def cached_schedule_in_db() -> bool:
        schedule_data, _, _, _, _, _ = load_schedule_from_db()
        return schedule_has_entries(schedule_data)

    def _apply_loaded_schedule(
        schedule_data,
        group_options,
        week_options,
        week_dates,
        *,
        ...
        state["parsed_schedule_per_group"] = schedule_data
        state["group_options"] = normalize_group_names(group_options or [])
        state["week_options"] = week_options or ["Цей тиждень", "Наступний тиждень"]
        state["week_dates"] = week_dates if week_dates else {}
        state["all_schedule_groups"] = normalize_group_names(group_options or [])
        state["schedule_from_cache"] = from_cache
        state["schedule_saved_at"] = saved_at or ""
        state["schedule_source_file"] = source_file or ""
        state["schedule_browsing"] = False
        if not state.get("selected_week"):
            state["selected_week"] = "Цей тиждень"

    def try_restore_cached_schedule(fallback_message: str | None = None) -> bool:
        schedule_data, group_options, week_options, week_dates, saved_at, source_file = load_schedule_from_db()
        if not schedule_has_entries(schedule_data):
            return False
        _apply_loaded_schedule(
            schedule_data,
            group_options,
            week_options,
            week_dates,
        ...
        show_snack(
            page,
            fallback_message or "Показано останній збережений розклад (може бути неактуальним).",
        )
        return True

    def open_cached_schedule(e):
        if try_restore_cached_schedule():
            update_content()

    def open_saved_schedule_view(e):
        if schedule_has_entries(state.get("parsed_schedule_per_group") or {}):
            state["schedule_browsing"] = False
            state["viewing_images"] = None
            update_content()

    def schedule_stale_banner() -> ft.Control | None:
        if not state.get("schedule_from_cache"):
            return None
        details: list[str] = []
        if state.get("schedule_saved_at"):
            details.append(f"завантажено {state['schedule_saved_at']}")
        if state.get("schedule_source_file"):
            details.append(f"файл «{state['schedule_source_file']}»")
        ...
        return ft.Container(
            ...
            content=ft.Row(
                [
                    ft.Icon(ft.icons.INFO_OUTLINE, color=ft.colors.AMBER_200, size=20),
                    ft.Text(
                        f"Цей розклад може бути неактуальним. Показано останній успішно завантажений розклад{detail_text}.",
                        size=14,
    ...
    def find_upcoming_entries() -> list[dict]:
        ...
        try:
            ...
            print(
                f"[NOTIFICATION] Перевірка: група={sg}, за {notify_time} хв + на початку пари, "
                f"сьогодні={today_date.strftime('%d.%m.%Y')}"
            )

            def _queue_class_notification(
                *,
                date_str: str,
                time_str: str,
                subject: str,
                ...
                key = (date_str, time_str, subject, kind)
                if key in state["notified_entries"]:
                    return
                title = "🔔 Нагадування про пару"
                if kind == NOTIF_KIND_START:
                    title = "▶️ Пара починається"
                try:
                    mark_notification_shown(date_str, time_str, subject, kind)
                    state["notified_entries"].add(key)
                    save_announcement(
                        msg,
                        title=title,
                        target_group=sg,
                        author_name="МНТУ Помічник",
                    )
                    telegram_bridge.notify(f"{title}\n{msg}")
                    messages.append({"message": msg, "title": title, "group": sg})
                    print(f"[NOTIFICATION] Збережено у «Повідомлення» + Telegram ({kind}): {msg}")
            ...
            for week_name, entries in group_schedule.items():
                ...
                for entry in entries:
                    ...
                    time_pre = (
                        notify_seconds <= total_seconds_until <= notify_seconds + pre_poll_slack
                        and total_seconds_until > 0
                    )
                    time_start = 0 < total_seconds_until <= start_poll_slack

                    if time_pre:
                        if minutes_until >= 60:
                            hours_left = minutes_until // 60
                            min_rem = minutes_until % 60
                            time_str_notify = (
                                f"{hours_left} год {min_rem} хв" if min_rem > 0 else f"{hours_left} год"
                            )
                        else:
                            time_str_notify = f"{minutes_until} хв"
                        pre_msg = (
                            f"Через {time_str_notify} починається пара ({time_str}) у групі {sg}: "
                            f"{subject}{teacher_text}"
                        )
                        _queue_class_notification(
                            date_str=date_str,
                            time_str=time_str,
                            subject=subject,
                            teacher=teacher,
                    ...
                    if time_start:
                        if minutes_until <= 1:
                            start_msg = (
                                f"Зараз починається пара ({time_str}) у групі {sg}: {subject}{teacher_text}"
                            )
                        else:
                            start_msg = (
                                f"Пара починається о {time_str} (через {minutes_until} хв) у групі {sg}: "
                                f"{subject}{teacher_text}"
                            )
                        _queue_class_notification(
                            date_str=date_str,
    ...
    def check_notifications(e):
        ...
        if notifications:
            update_content()
            show_snack(page, f"Додано {len(notifications)} нагадувань у «Повідомлення» та Telegram")
        else:
            show_snack(page, "Немає нових нагадувань про пари зараз")
    ...
    def schedule_page():
        if (
            not state.get("schedule_browsing", True)
            and state.get("week_options")
            ...
            stale_banner = schedule_stale_banner()
            if stale_banner is not None:
                content_list.append(stale_banner)
        ...
        def get_file_tiles():
            ...
            if not state["schedule_files"]:
                ...
                if cached_schedule_in_db():
                    tiles.append(
                        ft.Button(
                            "Відкрити збережений розклад",
                            icon=ft.icons.HISTORY,
                            on_click=open_cached_schedule,
                            style=ft.ButtonStyle(padding=_padding_symmetric(horizontal=20, vertical=12), shape=ft.RoundedRectangleBorder(radius=10)),
                        )
                    )
                return tiles
            ...
            if schedule_has_entries(state.get("parsed_schedule_per_group") or {}):
                details: list[str] = []
                if state.get("schedule_saved_at"):
                    details.append(state["schedule_saved_at"])
                if state.get("schedule_source_file"):
                    details.append(f"«{state['schedule_source_file']}»")
                subtitle = " · ".join(details) if details else "збережено локально"
                stale_note = (
                    " (може бути неактуальним)"
                    if state.get("schedule_from_cache")
                    else ""
                )
                tiles.append(
                    ft.Container(
                        content=ft.ListTile(
                            leading=ft.Icon(ft.icons.SCHEDULE, color=colors["ACCENT"], size=22),
                            title=ft.Text(
                                f"Останній завантажений розклад{stale_note}",
                                size=16,
                                color=colors["TEXT_COLOR"],
                            ),
                            subtitle=ft.Text(subtitle, size=13, color=colors["TEXT_MUTED"]),
                            on_click=open_saved_schedule_view,
                        ),
                        border_radius=12,
                        bgcolor=colors["BG_CARD"],
            ...
            async def load_content():
                ...
                try:
                    ...
                    if is_docx:
                        ...
                        if not p:
                            if try_restore_cached_schedule(
                                "Сайт недоступний. Показано останній збережений розклад."
                            ):
                                return
                            show_snack(
                                page,
                            ...
                            try:
                                ...
                                if not try_restore_cached_schedule(
                                    "Не вдалося завантажити файл. Показано останній збережений розклад."
                                ):
                        ...
                        if docx_bytes:
                            ...
                            if len(all_tables) >= 1:
                                ...
                                if not sorted_mondays:
                                    ...
                                    if try_restore_cached_schedule(
                                        f"{msg} Показано останній збережений розклад."
                                    ):
                                        state["schedule_loading"] = False
                                        update_content()
                                        return
                                ...
                                state["schedule_from_cache"] = False
                                state["schedule_browsing"] = False
                                state["schedule_source_file"] = file.name

                                try:
                                    saved_at = save_schedule_to_db(
                                        combined,
                                        normalize_group_names(all_groups),
                                        week_options,
                                        state["week_dates"],
                                        source_file_name=file.name,
                                    )
                                    state["schedule_saved_at"] = saved_at
                                except Exception as e:
                    ...
                    if not images and (docx_bytes is None or is_docx):
                        if not try_restore_cached_schedule(
                            "Не вдалося відкрити розклад. Показано останній збережений."
                        ):
                    ...
                    if not try_restore_cached_schedule(
                        f"Помилка відкриття файлу. Показано останній збережений розклад."
                    ):
    ...
    def calendar_page():
        ...
        def _safe_update(control: ft.Control) -> bool:
            try:
                if control.page:
                    control.update()
                    return True
    ...
    def settings_page():
        ...
        def on_switch_change(e):
            state["notifications_enabled"] = bool(e.control.value)
            save_settings_to_db(
                state["notifications_enabled"],
                state["notify_minutes"],
                state["notify_group"],
        ...
        def on_slider_change(e):
            ...
            notify_label.value = f"Нагадування за {state['notify_minutes']} хв до пари та на початку пари"
            notify_label.update()
        ...
        def open_bot_token_help(e):
            help_dialog = ft.AlertDialog(
                title=ft.Text("Як отримати токен бота", size=18, weight="bold"),
                content=ft.Container(
                    content=ft.Column(
                        [
                            ft.Text("1. Перейдіть у Telegram-бота @BotFather", size=14, color=colors["TEXT_COLOR"]),
                            ft.Text("2. Створіть бота за вказівками", size=14, color=colors["TEXT_COLOR"]),
                            ft.Text(
                                "3. Після створення бота ви отримаєте токен - "
                                "скопіюйте його та вставте в поле «Bot token».",
                                size=14,
                                color=colors["TEXT_COLOR"],
                            ),
                        ],
                ...
                actions=[
                    ft.Button(
                        "Зрозуміло",
                        on_click=lambda ev: setattr(help_dialog, "open", False) or page.update(),
                        bgcolor=colors["ACCENT"],
                        style=ft.ButtonStyle(
        ...
        def test_notifications_now(e):
            ...
            if notifications:
                ...
                show_snack(
                    page,
                    f"Додано {len(notifications)} нагадувань у вкладку «Повідомлення» та в Telegram",
                )
            else:
                sg = state.get("notify_group") or state.get("selected_group")
                if not sg:
        ...
        notify_label = ft.Text(
            f"Нагадування за {state['notify_minutes']} хв до пари та на початку пари"
        )

        telegram_section_items = [
        ...
        if state.get("telegram_enabled", False):
            telegram_section_items.extend([
                ft.Container(height=8),
                ft.Row(
                    [
                        ft.TextField(
                        ...
                        ft.Container(
                            content=ft.Icon(ft.icons.INFO_OUTLINE, color=colors["ACCENT_TEXT"], size=20),
                            width=40,
                            height=40,
                            border_radius=20,
                            bgcolor=colors["ACCENT"],
                            alignment=ft.alignment.center,
                            tooltip="Як отримати токен бота",
                            on_click=open_bot_token_help,
                            ink=True,
                        ),
                    ],
    ...
    def profile_page():
        ...
        if user_role == "Адміністратор":
            def open_users(e):
                users_column_ref = ft.Ref[ft.Column]()

                def on_search_change(ev):
                    rebuild_users_list()

                search_field = ft.TextField(label="Пошук (email, ПІБ)", width=420, on_change=on_search_change)

                def user_fio(u: dict) -> str:
                    return " ".join(
                        [(u.get("last_name") or "").strip(), (u.get("first_name") or "").strip(), (u.get("middle_name") or "").strip()]
                    ).strip()

                def rebuild_users_list():
                    if not users_column_ref.current:
                        return
                    q = (search_field.value or "").strip()
                    users = list_users(search=q)
                    rows = []
                    if not users:
                        rows.append(ft.Text("Користувачів не знайдено.", size=13, color=colors["TEXT_MUTED"]))
                    for u in users:
                        fio = user_fio(u) or "—"
                        rows.append(
                            ft.Container(
                                content=ft.ListTile(
                                    title=ft.Text(u["email"], size=14, color=colors["TEXT_COLOR"]),
                                    subtitle=ft.Text(f"{fio} · {u.get('role', 'Студент')}", size=12, color=colors["TEXT_MUTED"]),
                                ),
                                padding=4,
                                border_radius=8,
                                bgcolor=colors["BG_CARD"],
                                border=_border_all(1, colors["BORDER_COLOR"]),
                                on_click=lambda ev, usr=u: open_user_form(usr),
                                ink=True,
                            )
                        )
                    users_column_ref.current.controls = rows
                    users_column_ref.current.update()

                def open_user_form(edit_user: dict | None = None):
                    is_edit = edit_user is not None
                    email_field = ft.TextField(
                        label="Email",
                        value=(edit_user or {}).get("email", ""),
                        width=420,
                        read_only=is_edit,
                    )
                    last_name_field = ft.TextField(label="Прізвище", value=(edit_user or {}).get("last_name", ""), width=420)
                    first_name_field = ft.TextField(label="Ім'я", value=(edit_user or {}).get("first_name", ""), width=420)
                    middle_name_field = ft.TextField(label="По батькові", value=(edit_user or {}).get("middle_name", ""), width=420)
                    role_dd = ft.Dropdown(
                        label="Роль",
                        value=(edit_user or {}).get("role", "Студент") or "Студент",
                        width=420,
                        options=[
                    ...
                    def save_user_click(ev):
                        ...
                        if is_admin_email(email):
                            role = "Адміністратор"
                        upsert_user_profile(email, last_name, first_name, middle_name, role)
                        set_user_role_by_email(email, role)
                        if email == (state.get("email") or "").strip().lower():
                            state["user_role"] = role
                        ...
                        rebuild_users_list()
                        update_content()
                        show_snack(page, f"Профіль збережено: {email} ({role})")
                        telegram_bridge.notify(f"🛡 Профіль оновлено: {email}\n{fio_line(last_name, first_name, middle_name)}\nРоль: {role}")

                    def fio_line(ln, fn, mn):
                        return " ".join([ln, fn, mn]).strip()

                    def delete_user_click(ev):
                        email = (edit_user or {}).get("email", "").strip().lower()
                        if not email:
                            return
                        if email == (state.get("email") or "").strip().lower():
                            show_snack(page, "Не можна видалити власний профіль.")
                            return
                        if is_admin_email(email):
                            show_snack(page, "Не можна видалити системного адміністратора.")
                            return
                        if delete_user_by_email(email):
                            form_d.open = False
                            page.update()
                            rebuild_users_list()
                            show_snack(page, f"Профіль {email} видалено.")
                        else:
                            show_snack(page, "Не вдалося видалити профіль.")

                    form_actions = [
                        ft.TextButton("Скасувати", on_click=lambda ev: (setattr(form_d, "open", False), page.update())),
                        ft.Button(
                            "Зберегти",
                            on_click=save_user_click,
                            bgcolor=colors["ACCENT"],
                            style=ft.ButtonStyle(color=colors["ACCENT_TEXT"]),
                        ),
                    ]
                    if is_edit:
                        form_actions.insert(
                            0,
                            ft.TextButton("Видалити", on_click=delete_user_click),
                        )

                    form_d = ft.AlertDialog(
                        title=ft.Text("Редагувати профіль" if is_edit else "Новий профіль"),
                        content=ft.Column(
                            [
                                ft.Text(
                                    "Користувач отримає роль після входу з цим email у додаток.",
                                    size=12,
                                    color=colors["TEXT_MUTED"],
                                ),
                                ft.Container(height=8),
                    ...
                    page.overlay.append(form_d)
                    form_d.open = True
                    page.update()

                def open_create_user(ev):
                    open_user_form(None)
                ...
                d = ft.AlertDialog(
                    ...
                    content=ft.Column(
                        [
                            ft.Text(
                                "Створюйте та редагуйте профілі студентів, викладачів та адміністраторів.",
                                size=13,
                                color=colors["TEXT_MUTED"],
                            ),
                            ft.Container(height=8),
                            search_field,
                            ft.Container(
                                content=ft.Column(ref=users_column_ref, spacing=6, scroll=ft.ScrollMode.AUTO),
                                height=320,
                                width=440,
                            ),
                        ],
                    ...
                    actions=[
                        ft.TextButton("Закрити", on_click=lambda ev: (setattr(d, "open", False), page.update())),
                        ft.Button(
                            "Додати користувача",
                            icon=ft.icons.PERSON_ADD,
                            on_click=open_create_user,
                            bgcolor=colors["ACCENT"],
                            style=ft.ButtonStyle(color=colors["ACCENT_TEXT"]),
                        ),
                    ],
            ...
            def open_send_notification_admin(e):
                ...
                def publish_click(ev):
                    ...
                    if text or header:
                        ...
                        finish_announcement_notification(aid, telegram_text=bot_preview)
                        update_content()
            ...
            role_controls = [
                ...
                ft.Container(height=8),
                make_action_row(ft.icons.PEOPLE, "Профілі студентів та викладачів", open_users),
            ...
            def open_send_notification(e):
                ...
                def publish_click(ev):
                    ...
                    if text or header:
                        ...
                        finish_announcement_notification(aid, telegram_text="\n".join(bot_msg))
                        update_content()
    ...
    def render_auth_gate():
        ...
        def save_profile_and_continue(e):
            ...
            existing = get_user_profile_by_email(email)
            if existing:
                if not last_name and (existing.get("last_name") or "").strip():
                    last_name = (existing.get("last_name") or "").strip()
                if not first_name and (existing.get("first_name") or "").strip():
                    first_name = (existing.get("first_name") or "").strip()
                if not middle_name and (existing.get("middle_name") or "").strip():
                    middle_name = (existing.get("middle_name") or "").strip()
            if is_admin_email(email):
                final_role = "Адміністратор"
            elif existing and (existing.get("role") or "").strip() in ("Викладач", "Адміністратор"):
                final_role = (existing.get("role") or "Студент").strip()
            else:
                final_role = "Студент"
            ...
            need_role_request = (
                not is_admin_email(email)
                and requested_role != final_role
                and requested_role in ("Викладач", "Адміністратор")
            )
            if need_role_request:
                create_role_change_request(
                    email=email,
                    last_name=last_name,
            ...
            elif existing and final_role in ("Викладач", "Адміністратор"):
                show_snack(page, f"Профіль збережено. Ваша роль: {final_role}.")
            elif need_role_request:
                show_snack(page, "Профіль збережено. Заявка на зміну ролі відправлена адміну в Telegram.")
            else:
                show_snack(page, "Профіль збережено.")
    ...
    if state.get("auth_required", True):
        ...
        state["ui_ready"] = True
        update_content()
        if state.get("notifications_enabled", True):
            ...
            show_pending_announcements(page, True, target_group=current_target_group())
